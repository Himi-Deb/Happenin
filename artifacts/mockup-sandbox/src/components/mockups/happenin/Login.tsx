...
function AtmospherePanel() {
  ...
  const dur = 30;
  const perSlide = dur / total; // 5s per slide
  ...
  return (
    <div style={{ position: 'relative', width: '55%', height: '100vh', background: '#000', overflow: 'hidden', flexShrink: 0 }}>
      ...
      {/* Always-on dark overlay — keeps text readable over any image */}
      ...
      {/* Teal colour tint — brand identity over the photos */}
      ...
      {/* Per-slide category badge (top-left) */}
      {SLIDES.map((slide, i) => (
        <div
          key={i}
          style={{
