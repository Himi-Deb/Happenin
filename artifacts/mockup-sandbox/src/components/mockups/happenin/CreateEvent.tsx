  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 200, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', padding: '32px 56px 0', pointerEvents: 'none' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, pointerEvents: 'all' }}>
        <button style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, padding: '12px 22px', color: 'rgba(255,255,255,0.55)', fontFamily: F, fontSize: 15, fontWeight: 600, cursor: 'pointer' }}>
          Save Draft
        </button>
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
      ...
      <div
        ...
        style={{ width: 44, height: 24, borderRadius: 12, background: value ? TEAL : 'rgba(255,255,255,0.1)', border: `1px solid ${value ? TEAL : 'rgba(255,255,255,0.12)'}`, cursor: 'pointer', transition: 'all 0.2s', position: 'relative', flexShrink: 0 }}
      >
...
export function CreateEvent() {
  ...
  const [lineup, setLineup] = useState([
    { act: '', time: '', type: 'support' as 'support' | 'headline' | 'info' },
  ]);
  ...
  function addLineupRow() {
    setLineup(l => [...l, { act: '', time: '', type: 'support' }]);
  }
  ...
  function removeLineupRow(i: number) {
      ...
      <div style={{ marginTop: 125, flex: 1, display: 'flex', overflow: 'hidden', padding: '16px 20px 20px', gap: 16 }}>
              ...
              return (
                <div key={s.id} className="ce-nav-row" onClick={() => scrollToSection(s.id)}
                  ...
                  <div style={{ width: 30, height: 30, borderRadius: 9, background: isActive ? `${TEAL}14` : 'rgba(255,255,255,0.04)', border: `1px solid ${isActive ? `${TEAL}24` : 'rgba(255,255,255,0.07)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, color: isActive ? TEAL : 'rgba(255,255,255,0.3)' }}>
                    {isDone
                      ? <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke={isActive ? TEAL : 'rgba(127,224,213,0.5)'} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
                      : s.icon}
                  </div>
                  ...
                  {!isDone && <div style={{ width: 6, height: 6, borderRadius: '50%', background: 'rgba(255,255,255,0.1)', flexShrink: 0 }} />}
                </div>
              );
            })}
          </div>

          {/* publish button at bottom of sidebar */}
        ...
        <div className="scroll-y" style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 0 }}>
          ...
          <div id="section-basics" style={{ marginBottom: 20, borderRadius: 20, overflow: 'hidden', position: 'relative', height: 220, background: 'linear-gradient(135deg, #0f1e20, #1a1208)', border: '1px solid rgba(255,255,255,0.06)', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
            ...
            <div style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, padding: '8px 20px', fontSize: 13, fontWeight: 600, color: 'rgba(255,255,255,0.5)' }}>
              Browse Files
            </div>
          </div>
            ...
            <SectionCard title="Date & Time">
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14 }}>
                <div>
                  ...
                  <input className="ce-inp" type="date" value={date} onChange={e => setDate(e.target.value)} style={{ ...inp, colorScheme: 'dark' }} />
                </div>
                <div>
                  <FieldLabel>Start Time *</FieldLabel>
                  <input className="ce-inp" type="time" value={startTime} onChange={e => setStartTime(e.target.value)} style={{ ...inp, colorScheme: 'dark' }} />
                </div>
                <div>
                  <FieldLabel>End Time</FieldLabel>
                  <input className="ce-inp" type="time" value={endTime} onChange={e => setEndTime(e.target.value)} style={{ ...inp, colorScheme: 'dark' }} />
                </div>
              </div>
            ...
            <SectionCard title="Location">
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                <div>
                  ...
                  <input className="ce-inp" value={venue} onChange={e => setVenue(e.target.value)} placeholder="e.g. Rooftop Arena" style={inp} />
                </div>
                <div>
                  <FieldLabel>Street Address</FieldLabel>
                  <input className="ce-inp" value={address} onChange={e => setAddress(e.target.value)} placeholder="e.g. 32 Upper Ground" style={inp} />
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                  <div>
                    <FieldLabel>City *</FieldLabel>
                    <input className="ce-inp" value={city} onChange={e => setCity(e.target.value)} placeholder="e.g. London" style={inp} />
                  </div>
                  <div>
                    <FieldLabel>Postcode</FieldLabel>
                    <input className="ce-inp" placeholder="e.g. SE1 9PP" style={inp} />
                  </div>
                </div>
                ...
                <div style={{ height: 120, borderRadius: 14, background: '#0d1415', border: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, color: 'rgba(255,255,255,0.2)', fontSize: 13, cursor: 'pointer', overflow: 'hidden', position: 'relative' }}>
                  <svg viewBox="0 0 800 120" width="100%" height="100%" style={{ position: 'absolute', inset: 0 }}>
                    ...
                    <rect x="200" y="32" width="120" height="56" rx="4" fill="rgba(127,224,213,0.05)" stroke="rgba(127,224,213,0.08)" strokeWidth="1"/>
                    <rect x="340" y="40" width="200" height="40" rx="4" fill="rgba(127,224,213,0.05)" stroke="rgba(127,224,213,0.08)" strokeWidth="1"/>
          ...
          <div id="section-description">
            <SectionCard title="Description">
              <div>
                ...
                <textarea className="ce-inp" value={desc} onChange={e => setDesc(e.target.value)} placeholder="Tell people what to expect — the vibe, who's performing, what makes it special..." rows={7} style={{ ...inp, lineHeight: 1.7 }} />
              </div>
              <div style={{ marginTop: 14 }}>
                <FieldLabel>Accessibility Info</FieldLabel>
                <textarea className="ce-inp" placeholder="e.g. Wheelchair accessible, step-free entry via Waterloo Road entrance" rows={3} style={{ ...inp, lineHeight: 1.7 }} />
              </div>
            </SectionCard>
          </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {lineup.map((row, i) => (
                  <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 140px 120px auto', gap: 10, alignItems: 'center' }}>
                    ...
                    {lineup.length > 1 && (
                      <button onClick={() => removeLineupRow(i)} style={{ width: 36, height: 36, borderRadius: 10, background: 'rgba(255,100,100,0.07)', border: '1px solid rgba(255,100,100,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0 }}>
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="rgba(255,120,120,0.7)" strokeWidth="2.5" strokeLinecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                      </button>
                    )}
                    {lineup.length === 1 && <div style={{ width: 36 }} />}
                  </div>
                ))}
