...
const CHANNELS = [
  {
    ...
    badge: 2, readOnly: true, internal: false, color: '#7FE0D5',
  },
  {
    id: 'crew' as ChannelId,
    name: 'Organiser Crew',
    desc: 'Internal team channel — not visible to attendees.',
    badge: 4, readOnly: false, internal: true, color: '#EBE88A',
  },
  {
    id: 'general' as ChannelId,
    name: 'General Chat',
    desc: 'Open to all ticket holders and the organiser team.',
    badge: 0, readOnly: false, internal: false, color: '#B1D8D4',
  },
];
...
const RC: Record<string, { bg: string; text: string; label: string }> = {
  ...
  staff:     { bg: '#1e1a08', text: '#EBE88A', label: 'Staff' },
  admin:     { bg: '#0a1828', text: '#B1D8D4', label: 'Admin' },
  ...
  return (
    <div style={{ height: '100vh', background: '#080a0b', display: 'flex', flexDirection: 'column', overflow: 'hidden', fontFamily: F }}>
      <style>{`
        ...
        .msg-row:hover .mod-bar { opacity:1 !important; }
        ...
        <div style={{ borderRight: '1px solid rgba(255,255,255,0.07)', display: 'flex', flexDirection: 'column', background: 'rgba(0,0,0,0.15)', overflow: 'hidden' }}>
          ...
          <div style={{ flex: 1, padding: '0 20px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 10 }}>
            {CHANNELS.map(c => {
              ...
              return (
                <div key={c.id} className="ch-card" onClick={() => setActive(c.id)}
                  style={{ position: 'relative', padding: '18px 20px', borderRadius: 16, border: `1.5px solid ${isActive ? `${c.color}45` : 'rgba(255,255,255,0.07)'}`, background: isActive ? `${c.color}0a` : 'rgba(255,255,255,0.02)', cursor: 'pointer', transition: 'all 0.18s', boxShadow: isActive ? `0 0 24px ${c.color}10` : 'none' }}>
                  {/* Active left bar */}
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 5 }}>
                        ...
                        {c.internal && (
                          <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: '1px', color: '#EBE88A', background: 'rgba(235,232,138,0.1)', border: '1px solid rgba(235,232,138,0.2)', borderRadius: 5, padding: '2px 7px', textTransform: 'uppercase' }}>Internal</span>
                        )}
                        {c.readOnly && (
                          <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: '1px', color: '#7FE0D5', background: 'rgba(127,224,213,0.08)', border: '1px solid rgba(127,224,213,0.18)', borderRadius: 5, padding: '2px 7px', textTransform: 'uppercase' }}>Read-only</span>
                        )}
                        {c.badge > 0 && (
                          <div style={{ marginLeft: 'auto', background: c.color, borderRadius: 9999, minWidth: 20, height: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 5px' }}>
          ...
          {/* Moderation panel at bottom */}
          <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', padding: '18px 24px 20px', flexShrink: 0 }}>
            <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '2.5px', textTransform: 'uppercase', color: 'rgba(255,255,255,0.2)', marginBottom: 14 }}>Moderation</div>
            {[
              { label: 'Slow mode (30s)', val: slowMode, set: setSlowMode, on: '#7FE0D5' },
              { label: 'Mute channel',    val: muteChat, set: setMuteChat, on: '#EBE88A' },
            ].map(t => (
              <div key={t.label} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.4)', fontWeight: 500 }}>{t.label}</span>
                <div onClick={() => t.set(!t.val)} style={{ width: 38, height: 21, borderRadius: 11, background: t.val ? t.on : 'rgba(255,255,255,0.1)', cursor: 'pointer', position: 'relative', transition: 'background 0.2s', flexShrink: 0 }}>
                  <div style={{ position: 'absolute', top: 3, left: t.val ? 19 : 3, width: 15, height: 15, borderRadius: '50%', background: '#fff', transition: 'left 0.2s', boxShadow: '0 1px 4px rgba(0,0,0,0.4)' }} />
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <div style={{ padding: '16px 40px', borderBottom: '1px solid rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', gap: 14 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                {[
                  ...
                  <div key={i} style={{ width: 34, height: 34, borderRadius: 9, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: 'rgba(255,255,255,0.3)' }}>{ico}</div>
                ))}
              </div>
            </div>
          </div>
          ...
          <div className="feed" style={{ flex: 1, overflowY: 'auto', padding: '20px 0 8px' }}>
            ...
            <div style={{ margin: '0 40px 20px', background: `${ch.color}07`, border: `1px solid ${ch.color}16`, borderRadius: 12, padding: '11px 18px', display: 'flex', gap: 12, alignItems: 'flex-start' }}>
              ...
              <span style={{ fontSize: 13, color: `${ch.color}80`, lineHeight: 1.6 }}>
            ...
            {msgs.map(m => (
              <div key={m.id} className="msg-row" onMouseEnter={() => setHovered(m.id)} onMouseLeave={() => setHovered(null)}
                style={{ position: 'relative', display: 'flex', gap: 16, padding: '11px 40px', background: hovered === m.id ? 'rgba(255,255,255,0.016)' : 'transparent', transition: 'background 0.12s' }}>
                <Av ini={m.ini} role={m.role} bg={m.ac} size={42}/>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 5, flexWrap: 'wrap' }}>
                    <span style={{ fontSize: 15, fontWeight: 700, color: m.role === 'organiser' ? '#7FE0D5' : m.role === 'staff' ? '#EBE88A' : m.role === 'admin' ? '#B1D8D4' : 'rgba(255,255,255,0.8)', letterSpacing: '-0.2px' }}>{m.name}</span>
                    {m.role !== 'attendee' && (
                      <span style={{ fontSize: 9.5, fontWeight: 700, letterSpacing: '0.7px', color: RC[m.role]?.text, background: `${RC[m.role]?.text}18`, border: `1px solid ${RC[m.role]?.text}28`, borderRadius: 5, padding: '2px 7px', textTransform: 'uppercase' }}>{RC[m.role]?.label}</span>
                    )}
                  ...
                  {/* Image attachment */}
                  {m.hasImg && (
                    <div style={{ marginTop: 12, background: 'linear-gradient(135deg,#0d2830,#0e3a3e,#091e22)', border: `1px solid ${ch.color}18`, borderRadius: 14, width: 360, height: 190, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 10, cursor: 'pointer', position: 'relative', overflow: 'hidden' }}>
                      <div style={{ position: 'absolute', inset: 0, backgroundImage: 'repeating-linear-gradient(0deg,rgba(127,224,213,0.035) 0,rgba(127,224,213,0.035) 1px,transparent 1px,transparent 40px),repeating-linear-gradient(90deg,rgba(127,224,213,0.035) 0,rgba(127,224,213,0.035) 1px,transparent 1px,transparent 40px)' }} />
                      <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
                        <div style={{ width: 48, height: 48, borderRadius: 14, background: `${ch.color}15`, border: `1px solid ${ch.color}30`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={ch.color} strokeWidth="1.8" strokeLinecap="round"><rect x="3" y="3" width="18" height="18" rx="3"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="M21 15l-5-5L5 21"/></svg>
                        </div>
                        <span style={{ fontSize: 13.5, fontWeight: 600, color: `${ch.color}90` }}>{m.imgCaption}</span>
                      </div>
                    </div>
                  )}

                  {/* Read count (announcements) */}
                  {m.readers && (
                    <div style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 6 }}>
                      ...
                      <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.2)', fontWeight: 500 }}>{m.readers.toLocaleString()} read</span>
                    </div>
                  )}
                </div>

                {/* Hover mod toolbar */}
                <div className="mod-bar" style={{ opacity: 0, transition: 'opacity 0.12s', position: 'absolute', top: 8, right: 36, display: 'flex', gap: 2, background: '#111416', border: '1px solid rgba(255,255,255,0.09)', borderRadius: 10, padding: '4px 5px', boxShadow: '0 4px 16px rgba(0,0,0,0.4)' }}>
                  {[
                    { t: 'Pin',    d: 'M12 2v20M5 9l7-7 7 7' },
                    { t: 'Reply',  d: 'M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z' },
                  ].map(a => (
                    <button key={a.t} title={a.t} style={{ background: 'none', border: 'none', padding: '5px 7px', borderRadius: 7, color: 'rgba(255,255,255,0.4)', cursor: 'pointer', display: 'flex', alignItems: 'center' }}>
                      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d={a.d}/></svg>
                    </button>
                  ))}
          ...
          <div style={{ padding: '14px 36px 22px', flexShrink: 0 }}>
            {ch.readOnly && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                ...
                <span style={{ fontSize: 12, fontWeight: 600, color: 'rgba(235,232,138,0.45)' }}>Posting as Organiser — attendees cannot reply in this channel</span>
              </div>
            )}
