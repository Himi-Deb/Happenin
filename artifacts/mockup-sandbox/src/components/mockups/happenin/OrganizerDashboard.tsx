  ...
  return (
    <div style={{ background: 'rgba(255,255,255,0.025)', border: `1px solid ${color}20`, borderRadius: 20, padding: '28px 32px', position: 'relative', overflow: 'hidden', flexShrink: 0 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20 }}>
        <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase', color: 'rgba(255,255,255,0.3)' }}>{label}</span>
      ...
      <div style={{ marginTop: 125, flexShrink: 0 }}>
        ...
        <div style={{ padding: '0 56px 32px', display: 'flex', alignItems: 'center', gap: 28, borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
              ...
              <div style={{ background: 'rgba(127,224,213,0.1)', border: '1px solid rgba(127,224,213,0.25)', borderRadius: 8, padding: '4px 12px', flexShrink: 0 }}>
                <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: '1.5px', textTransform: 'uppercase', color: '#7FE0D5' }}>Live</span>
              </div>
            </div>
          ...
          <div style={{ display: 'flex', gap: 12, flexShrink: 0 }}>
            <button className="action-btn" style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '14px 24px', borderRadius: 14, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.7)', fontFamily: F, fontSize: 15, fontWeight: 600, cursor: 'pointer', transition: 'all 0.18s' }}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
              Edit Event
            </button>
            <button className="action-primary" style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '14px 28px', borderRadius: 14, background: '#7FE0D5', border: 'none', color: '#0e2a2c', fontFamily: F, fontSize: 15, fontWeight: 700, cursor: 'pointer', transition: 'opacity 0.18s' }}>
        ...
        <div style={{ padding: '0 56px', display: 'flex', gap: 0, borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
          {(['overview', 'attendees', 'checklist'] as const).map(t => (
            <button key={t} className="dash-tab" onClick={() => setTab(t)} style={{ background: 'none', border: 'none', borderBottom: `2px solid ${tab === t ? '#7FE0D5' : 'transparent'}`, padding: '18px 28px 16px', fontFamily: F, fontSize: 14, fontWeight: tab === t ? 700 : 500, color: tab === t ? '#7FE0D5' : 'rgba(255,255,255,0.35)', cursor: 'pointer', transition: 'color 0.15s', textTransform: 'capitalize', letterSpacing: '0.2px' }}>
              {t}
            </button>
          ))}
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '36px 56px 48px' }} className="scroll-y">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 20, marginBottom: 36 }}>
            <StatCard
              ...
              sub={`${Math.round(claimed / capacity * 100)}% of ${capacity.toLocaleString()} capacity`}
              color="#7FE0D5"
              icon={<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#7FE0D5" strokeWidth="2" strokeLinecap="round"><path d="M2 9a3 3 0 0 1 0 6v2a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-2a3 3 0 0 1 0-6V7a2 2 0 0 0-2-2H4a2 2 0 0 0-2 2v2z"/></svg>}
            />
            <StatCard
              label="Checked In"
              value={checkedIn.toLocaleString()}
              sub={`${Math.round(checkedIn / claimed * 100)}% of ticket holders`}
              color="#B1D8D4"
              icon={<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#B1D8D4" strokeWidth="2" strokeLinecap="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>}
            />
            <StatCard
              label="Chat Messages"
              value={msgs.toLocaleString()}
              sub="across 2 channels today"
              color="#EBE88A"
              icon={<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#EBE88A" strokeWidth="2" strokeLinecap="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>}
            />
            <StatCard
              label="Announcements"
              value="5"
              sub="842 avg readers per post"
              color="#7FE0D5"
              icon={<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#7FE0D5" strokeWidth="2" strokeLinecap="round"><path d="M22 2L11 13"/><path d="M22 2L15 22l-4-9-9-4 19-7z"/></svg>}
            />
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 24 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              ...
              <div style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 20, padding: '24px 24px 20px' }}>
                <span style={{ fontSize: 14, fontWeight: 700, color: 'rgba(255,255,255,0.5)', letterSpacing: '1.5px', textTransform: 'uppercase', fontSize: 11, display: 'block', marginBottom: 16 }}>Quick Actions</span>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {[
                    { label: 'Post Announcement', color: '#7FE0D5', icon: <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round"><path d="M22 2L11 13"/><path d="M22 2L15 22l-4-9-9-4 19-7z"/></svg> },
                    { label: 'Open Event Hub',    color: '#7FE0D5', icon: <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg> },
                    { label: 'View Attendees',   color: 'rgba(255,255,255,0.6)', icon: <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg> },
                    { label: 'Download Check-in List', color: 'rgba(255,255,255,0.6)', icon: <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg> },
                  ].map((qa, i) => (
                    <button key={i} className="action-btn" style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', borderRadius: 12, background: i < 2 ? 'rgba(127,224,213,0.06)' : 'rgba(255,255,255,0.03)', border: `1px solid ${i < 2 ? 'rgba(127,224,213,0.18)' : 'rgba(255,255,255,0.07)'}`, color: qa.color, fontFamily: F, fontSize: 14, fontWeight: 600, cursor: 'pointer', transition: 'all 0.18s', textAlign: 'left' }}>
                      {qa.icon}
                      {qa.label}
                    </button>
                  ))}
              ...
              <div style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 20, padding: '24px 24px 20px', flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                  <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: '1.5px', textTransform: 'uppercase', color: 'rgba(255,255,255,0.35)' }}>Pre-event Checklist</span>
                ...
                <div style={{ marginTop: 18, display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {CHECKLIST.map((item, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <div style={{ width: 20, height: 20, borderRadius: 6, background: item.done ? '#7FE0D5' : 'transparent', border: `1.5px solid ${item.done ? '#7FE0D5' : 'rgba(255,255,255,0.15)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'all 0.18s' }}>
          ...
          <div style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 20, overflow: 'hidden' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 160px 140px 120px', padding: '16px 32px', borderBottom: '1px solid rgba(255,255,255,0.07)', background: 'rgba(255,255,255,0.015)' }}>
              {['Attendee', 'Ticket', 'Status', 'Claimed'].map(h => (
                <span key={h} style={{ fontSize: 11, fontWeight: 700, letterSpacing: '1.5px', textTransform: 'uppercase', color: 'rgba(255,255,255,0.3)' }}>{h}</span>
              ))}
            </div>
            {[
              ...
              <div key={i} className="activity-row" style={{ display: 'grid', gridTemplateColumns: '1fr 160px 140px 120px', padding: '16px 32px', borderBottom: '1px solid rgba(255,255,255,0.04)', transition: 'background 0.12s', alignItems: 'center' }}>
                ...
                <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.3)' }}>{a.time}</span>
              </div>
            ))}
          </div>
        </div>
