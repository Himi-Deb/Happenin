    ...
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 200, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', padding: '32px 56px 0', pointerEvents: 'none' }}>
      <div style={{ pointerEvents: 'all', background: 'rgba(177,216,212,0.13)', borderRadius: 16, display: 'flex', alignItems: 'center', gap: 20, paddingRight: 28 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ color: 'rgba(255,255,255,0.4)', fontFamily: F, fontSize: 15 }}>
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ verticalAlign: 'middle', marginBottom: 1 }}><path d="M15 18l-6-6 6-6"/></svg>
          </span>
      ...
      {anyUnsaved && !saved && (
        <div style={{ position: 'fixed', bottom: 28, left: '50%', transform: 'translateX(-50%)', zIndex: 300, background: '#13110d', border: '1px solid rgba(235,232,138,0.3)', borderRadius: 16, padding: '14px 24px', display: 'flex', alignItems: 'center', gap: 16, boxShadow: '0 8px 32px rgba(0,0,0,0.5)' }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button onClick={() => { setChangedBasics(false); setChangedDateTime(false); setChangedDesc(false); setSaved(true); setTimeout(() => setSaved(false), 2500); }} style={{ background: TEAL, border: 'none', borderRadius: 10, padding: '8px 20px', fontFamily: F, fontSize: 13, fontWeight: 700, color: '#0e2a2c', cursor: 'pointer' }}>Save Changes</button>
          </div>
        </div>
      )}
      ...
      <div style={{ marginTop: 125, flex: 1, display: 'flex', overflow: 'hidden', padding: '16px 20px 20px', gap: 16 }}>
        ...
        <div style={{ width: 240, minWidth: 240, background: CARD_BG, borderRadius: 20, border: '1px solid rgba(255,255,255,0.06)', display: 'flex', flexDirection: 'column', overflow: 'hidden', flexShrink: 0 }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '10px 10px' }}>
            {SECTIONS.map(s => {
              ...
              const hasChange =
                (s.id === 'basics' && changedBasics) ||
                (s.id === 'datetime' && changedDateTime) ||
                (s.id === 'description' && changedDesc);
              return (
                <div key={s.id} className="ce-nav-row" onClick={() => scrollToSection(s.id)}
                  style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 14px', borderRadius: 13, background: isActive ? `${TEAL}10` : 'transparent', border: `1px solid ${isActive ? `${TEAL}28` : 'transparent'}`, cursor: 'pointer', transition: 'all 0.15s', marginBottom: 3, position: 'relative' }}>
                  ...
                  {hasChange && <div style={{ width: 7, height: 7, borderRadius: '50%', background: GOLD, flexShrink: 0 }} />}
                </div>
              );
            })}
          </div>
          ...
          <div style={{ padding: '14px 14px 16px', borderTop: '1px solid rgba(255,255,255,0.05)', flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 8 }}>
            <button
              onClick={() => { setChangedBasics(false); setChangedDateTime(false); setChangedDesc(false); setSaved(true); setTimeout(() => setSaved(false), 2500); }}
              style={{ width: '100%', background: anyUnsaved ? TEAL : 'rgba(127,224,213,0.1)', border: `1px solid ${anyUnsaved ? 'transparent' : 'rgba(127,224,213,0.15)'}`, borderRadius: 13, padding: '15px 0', fontFamily: F, fontSize: 15, fontWeight: 700, color: anyUnsaved ? '#0e2a2c' : 'rgba(127,224,213,0.5)', cursor: 'pointer', transition: 'all 0.2s', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
              Save Changes
            </button>
            <button style={{ width: '100%', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 13, padding: '12px 0', fontFamily: F, fontSize: 14, fontWeight: 600, color: 'rgba(255,255,255,0.3)', cursor: 'pointer' }}>
              Preview Event Page
            </button>
          </div>
        </div>

        {/* ── RIGHT: form scroll area ── */}
        <div className="scroll-y" style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 0 }}>
          ...
          <div id="section-basics" style={{ marginBottom: 20, borderRadius: 20, overflow: 'hidden', position: 'relative', height: 220, cursor: 'pointer' }}>
            {/* existing event art placeholder */}
            ...
            <div style={{ position: 'absolute', inset: 0 }}>
              <svg width="100%" height="100%" viewBox="0 0 1200 220" preserveAspectRatio="none">
                ...
                <circle cx="200" cy="110" r="180" fill="rgba(127,224,213,0.04)"/>
                <circle cx="900" cy="110" r="200" fill="rgba(235,232,138,0.03)"/>
            ...
            <div style={{ position: 'absolute', bottom: 20, left: 24, right: 24, display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
              ...
              <button style={{ background: 'rgba(0,0,0,0.5)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: 10, padding: '9px 18px', fontFamily: F, fontSize: 13, fontWeight: 600, color: 'rgba(255,255,255,0.6)', cursor: 'pointer', backdropFilter: 'blur(8px)' }}>
                Change Image
              </button>
            </div>
          </div>
          ...
          <SectionCard title="Basic Info" id="section-basics" changed={changedBasics}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
              <div>
                ...
                <input className="ce-inp" value={name} onChange={e => { setName(e.target.value); setChangedBasics(true); }} style={{ ...inp, fontSize: 18, fontWeight: 600, letterSpacing: '-0.3px' }} />
              </div>
              <div>
                <FieldLabel>Tagline</FieldLabel>
                <input className="ce-inp" value={tagline} onChange={e => { setTagline(e.target.value); setChangedBasics(true); }} style={inp} />
              </div>
              <div>
          ...
          <SectionCard title="Date & Time" id="section-datetime" changed={changedDateTime}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14 }}>
              <div>
                ...
                <input className="ce-inp" type="date" defaultValue="2025-06-14" onChange={() => setChangedDateTime(true)} style={{ ...inp, colorScheme: 'dark' }} />
              </div>
              <div>
                <FieldLabel>Start Time *</FieldLabel>
                <input className="ce-inp" type="time" defaultValue="17:30" onChange={() => setChangedDateTime(true)} style={{ ...inp, colorScheme: 'dark' }} />
              </div>
              <div>
                <FieldLabel>End Time</FieldLabel>
                <input className="ce-inp" type="time" defaultValue="23:00" onChange={() => setChangedDateTime(true)} style={{ ...inp, colorScheme: 'dark' }} />
              </div>
            </div>
          ...
          <SectionCard title="Location" id="section-location">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div>
                ...
                <input className="ce-inp" defaultValue="Rooftop Arena" style={inp} />
              </div>
              <div>
                <FieldLabel>Street Address</FieldLabel>
                <input className="ce-inp" defaultValue="32 Upper Ground" style={inp} />
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                <div>
                  <FieldLabel>City *</FieldLabel>
                  <input className="ce-inp" defaultValue="London" style={inp} />
                </div>
                <div>
                  <FieldLabel>Postcode</FieldLabel>
                  <input className="ce-inp" defaultValue="SE1 9PP" style={inp} />
                </div>
              </div>
              {/* map placeholder */}
              <div style={{ height: 120, borderRadius: 14, background: '#0d1415', border: '1px solid rgba(255,255,255,0.06)', overflow: 'hidden', position: 'relative', cursor: 'pointer' }}>
                <svg viewBox="0 0 800 120" width="100%" height="100%" style={{ position: 'absolute', inset: 0 }}>
                  ...
                  <rect x="200" y="32" width="120" height="56" rx="4" fill="rgba(127,224,213,0.06)" stroke="rgba(127,224,213,0.1)" strokeWidth="1"/>
                  <rect x="340" y="40" width="200" height="40" rx="4" fill="rgba(127,224,213,0.06)" stroke="rgba(127,224,213,0.1)" strokeWidth="1"/>
          ...
          <SectionCard title="Description" id="section-description" changed={changedDesc}>
            <div>
              <FieldLabel>About this Event *</FieldLabel>
              <textarea className="ce-inp" value={desc} onChange={e => { setDesc(e.target.value); setChangedDesc(true); }} rows={7} style={{ ...inp, lineHeight: 1.7 }} />
            </div>
            <div style={{ marginTop: 14 }}>
              <FieldLabel>Accessibility Info</FieldLabel>
              <textarea className="ce-inp" defaultValue="Wheelchair accessible via Waterloo Road entrance. Step-free throughout. BSL interpretation available on request." rows={3} style={{ ...inp, lineHeight: 1.7 }} />
            </div>
          </SectionCard>
          ...
          <SectionCard title="Tickets" id="section-tickets">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              <div>
                ...
                <input className="ce-inp" defaultValue="General Admission" style={inp} />
              </div>
              <div>
                <FieldLabel>Capacity</FieldLabel>
                <input className="ce-inp" type="number" defaultValue="1200" style={inp} />
              </div>
            </div>
            {/* live stats pill */}
            <div style={{ marginTop: 16, display: 'flex', gap: 12 }}>
              {[
                { label: 'Claimed', val: '842', color: TEAL },
                { label: 'Remaining', val: '358', color: 'rgba(255,255,255,0.4)' },
                { label: 'Checked In', val: '312', color: '#B1D8D4' },
              ].map(s => (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20, marginBottom: 16 }}>
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: 10, marginBottom: 10 }}>
                  {[
                    { ini: 'SJ', name: 'Sarah J.', role: 'Staff', bg: '#0e2a2c' },
                    { ini: 'MT', name: 'Marcus T.', role: 'Staff', bg: '#1a1208' },
                  ].map((co, i) => (
