...
function AtmospherePanel() {
  ...
  return (
    <div style={{ position: 'relative', width: '55%', height: '100vh', background: '#000', overflow: 'hidden', flexShrink: 0 }}>
      ...
      `}</style>
      {SLIDES.map((slide, i) => (
        <div key={i} style={{ position: 'absolute', inset: 0, opacity: 0, animation: `ev-slide ${dur}s ease-in-out ${i * perSlide}s infinite` }}>
          <div style={{ position: 'absolute', inset: '-5%', backgroundImage: `url(${slide.img})`, backgroundSize: 'cover', backgroundPosition: 'center', animation: `ev-zoom ${dur}s ease-in-out ${i * perSlide}s infinite alternate` }} />
        </div>
      ))}
      ...
      {SLIDES.map((slide, i) => (
        <div key={i} style={{ position: 'absolute', top: 36, left: 36, display: 'flex', alignItems: 'center', gap: 10, background: 'rgba(0,0,0,0.45)', backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 100, padding: '8px 16px 8px 12px', opacity: 0, animation: `ev-slide ${dur}s ease-in-out ${i * perSlide}s infinite` }}>
      ...
      <div style={{ position: 'absolute', top: 52, right: 36, display: 'flex', gap: 6 }}>
        {SLIDES.map((_, i) => (
          <div key={i} style={{ position: 'relative', width: 24, height: 2, borderRadius: 2, background: 'rgba(255,255,255,0.2)', overflow: 'hidden' }}>
            <div style={{ position: 'absolute', inset: 0, borderRadius: 2, background: '#7FE0D5', opacity: 0, animation: `ev-slide ${dur}s ease-in-out ${i * perSlide}s infinite` }} />
          </div>
        ))}
      </div>
        ...
        <div className="lf-form-in" style={{ width: '100%', maxWidth: 400, display: 'flex', flexDirection: 'column', gap: 0 }}>
          <div style={{ marginBottom: 36 }}>
            ...
            <p style={{ fontFamily: F, fontSize: 15, color: 'rgba(255,255,255,0.35)', margin: '10px 0 0', fontWeight: 400 }}>
              New here?{' '}
              <span onClick={() => navigate('/signup')} style={{ color: '#7FE0D5', textDecoration: 'none', fontWeight: 600, cursor: 'pointer' }}>Create an account</span>
            </p>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginBottom: 28 }}>
            ...
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                ...
                <span style={{ fontFamily: F, fontSize: 12, color: 'rgba(127,224,213,0.7)', fontWeight: 500, cursor: 'pointer' }}>Forgot password?</span>
              </div>
              <div style={{ position: 'relative' }}>
                <input type={showPw ? 'text' : 'password'} placeholder="••••••••" value={password} onChange={e => setPassword(e.target.value)} onFocus={() => setFocusedField('password')} onBlur={() => setFocusedField(null)} style={{ ...inputStyle('password'), paddingRight: 48 }} />
          ...
          <button className="lf-btn-primary" onClick={() => navigate('/confirmation')} style={{ width: '100%', background: '#7FE0D5', border: 'none', borderRadius: 14, padding: '16px 0', color: '#0e2a2c', fontFamily: F, fontSize: 15, fontWeight: 800, cursor: 'pointer', letterSpacing: '0.2px', transition: 'background 0.15s', marginBottom: 24 }}>
            Sign in
          </button>
          <p style={{ fontFamily: F, fontSize: 12, color: 'rgba(255,255,255,0.2)', textAlign: 'center', lineHeight: 1.7, margin: 0 }}>
            By continuing you agree to our{' '}
            <span style={{ color: 'rgba(255,255,255,0.4)', textDecoration: 'underline', cursor: 'pointer' }}>Terms of Service</span>
            {' '}and{' '}
