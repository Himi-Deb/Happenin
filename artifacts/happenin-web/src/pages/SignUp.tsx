...
function AtmospherePanel() {
  ...
  return (
    <div style={{ position: 'relative', width: '55%', height: '100vh', background: '#000', overflow: 'hidden', flexShrink: 0 }}>
      ...
      `}</style>
      {SLIDES.map((slide, i) => (
        <div key={i} style={{ position: 'absolute', inset: 0, opacity: 0, animation: `su-ev-slide ${dur}s ease-in-out ${i * perSlide}s infinite` }}>
          <div style={{ position: 'absolute', inset: '-5%', backgroundImage: `url(${slide.img})`, backgroundSize: 'cover', backgroundPosition: 'center', animation: `su-ev-zoom ${dur}s ease-in-out ${i * perSlide}s infinite alternate` }} />
        </div>
      ))}
      ...
      {SLIDES.map((slide, i) => (
        <div key={i} style={{ position: 'absolute', top: 36, left: 36, display: 'flex', alignItems: 'center', gap: 10, background: 'rgba(0,0,0,0.45)', backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 100, padding: '8px 16px 8px 12px', opacity: 0, animation: `su-ev-slide ${dur}s ease-in-out ${i * perSlide}s infinite` }}>
      ...
      <div style={{ position: 'absolute', top: 52, right: 36, display: 'flex', gap: 6 }}>
        {SLIDES.map((_, i) => (
          <div key={i} style={{ position: 'relative', width: 24, height: 2, borderRadius: 2, background: 'rgba(255,255,255,0.2)', overflow: 'hidden' }}>
            <div style={{ position: 'absolute', inset: 0, borderRadius: 2, background: '#EBE88A', opacity: 0, animation: `su-ev-slide ${dur}s ease-in-out ${i * perSlide}s infinite` }} />
          </div>
        ))}
      </div>
...
export default function SignUp() {
  ...
  const inputStyle = (fieldName: string): React.CSSProperties => ({
    ...
    border: `1px solid ${focusedField === fieldName ? 'rgba(127,224,213,0.5)' : 'rgba(255,255,255,0.1)'}`,
    borderRadius: 14,
    padding: '15px 18px',
    color: '#fff',
    fontFamily: F,
        ...
        <div className="su-form-in" style={{ width: '100%', maxWidth: 400, display: 'flex', flexDirection: 'column' }}>
          <div style={{ marginBottom: 32 }}>
            ...
            <p style={{ fontFamily: F, fontSize: 15, color: 'rgba(255,255,255,0.35)', margin: '10px 0 0', fontWeight: 400 }}>
              Already have one?{' '}
              <span onClick={() => navigate('/login')} style={{ color: '#7FE0D5', textDecoration: 'none', fontWeight: 600, cursor: 'pointer' }}>Sign in</span>
            </p>
          </div>
          <div style={{ display: 'flex', gap: 12, marginBottom: 28 }}>
            {[
              { label: 'Google', icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/></svg> },
              { label: 'Apple', icon: <svg width="17" height="17" viewBox="0 0 24 24" fill="currentColor" style={{ color: '#fff' }}><path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.8-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z"/></svg> },
            ].map(s => (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginBottom: 26 }}>
            <div>
              ...
              <input type="text" placeholder="Alex Morgan" value={name} onChange={e => setName(e.target.value)} onFocus={() => setFocusedField('name')} onBlur={() => setFocusedField(null)} style={inputStyle('name')} />
            </div>
            <div>
            ...
            <div>
              ...
              <div style={{ position: 'relative' }}>
                <input type={showPw ? 'text' : 'password'} placeholder="Min. 8 characters" value={password} onChange={e => setPassword(e.target.value)} onFocus={() => setFocusedField('password')} onBlur={() => setFocusedField(null)} style={{ ...inputStyle('password'), paddingRight: 48 }} />
          ...
          <button className="su-btn-primary" onClick={() => navigate('/login')} style={{ width: '100%', background: '#7FE0D5', border: 'none', borderRadius: 14, padding: '16px 0', color: '#0e2a2c', fontFamily: F, fontSize: 15, fontWeight: 800, cursor: 'pointer', letterSpacing: '0.2px', transition: 'background 0.15s', marginBottom: 22 }}>
            Create account
          </button>
          <p style={{ fontFamily: F, fontSize: 12, color: 'rgba(255,255,255,0.2)', textAlign: 'center', lineHeight: 1.7, margin: 0 }}>
            By creating an account you agree to our{' '}
            <span style={{ color: 'rgba(255,255,255,0.4)', textDecoration: 'underline', cursor: 'pointer' }}>Terms</span>
            {' '}and{' '}
